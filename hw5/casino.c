...
int read_int(){
    char buf[0x10];
    __read_chk( 0 , buf , 0xf , 0x10 ); // __read_chk(int fd, void* buf, size_t nbytes, size_t buflen)
    /*
    The interface __read_chk() shall function in the same way as the interface read(), 
    
    except that __read_chk() shall check for buffer overflow before computing a result. 
    
    If an overflow is anticipated, the function shall abort and the program calling it shall exit.
